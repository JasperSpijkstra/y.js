...
(function (window) {
    ...
    y.track = function () {
        var sessionStorage = window.sessionStorage,
            ...
            sessionNumber = cookie.get('sessnr'),
            ...
            userFrequency = cookie.get('yufq'),
            sessionFrequency = sessionStorage.getItem('ysfq');
            sessionNumber++,sessionFrequency++,userFrequency++;
        var userEngagementRatio = userFrequency / sessionNumber;
        ...
        cookie.set('yuer', userEngagementRatio, 30, host);
        cookie.set('yhist', history.split(cookie.get('yhist'), '|').add(trafficSource).join('|'), 30, host);
        
        if (!sessid) {
            sessid = session();
            sessionTime = 1;
            cookie.set('sessnr', sessionNumber, 30, host);
            ...
            sessionTime =  Math.floor(Date.now() / 1000) - sessionStorage.getItem('ysesst');
        }

        sessionStorage.setItem('ysfq', sfq);
        var sessionEngagementRatio = sessionFrequency / sessionTime;
        cookie.set('yser', sessionEngagementRatio, 30, host);

        var data = merge([y.data, params]);
        data.ufq = userFrequency;
        data.sfq = sessionFrequency;
        data.uer = userEngagementRatio;
        data.ser = sessionEngagementRatio;
        data.sessid = sessid;
